...
(function () {
  ...
  // Yahoo Finance's unofficial chart endpoint is used as the free data
  // source. Browsers can't call it directly (no CORS headers from Yahoo),
  // so requests are routed through a public CORS proxy, tried in order
  // until one works. None of these check the page's origin, so they also
  // work when this page is opened straight from disk (file://).
  // corsproxy.io was dropped from this list: its free tier now rejects most
  // localhost/file:// origins in practice (despite documentation saying
  // otherwise) and pushes a pricing page instead of proxying the request.
  // These two don't gate by origin at all. Ordered with the one that's
  // currently more reliable first — swap the order if that changes, since
  // free unofficial proxies come and go in reliability over time.
  const CORS_PROXIES = [
  ...
  const DEFAULT_SETTINGS = {
    ...
    useProxy: true, // Yahoo sends no CORS header, so this is required in virtually all browsers — off is for testing only
  ...
  function loadTimeframe(stock, tfKey) {
    ...
    const url = source.buildUrl(symbol, interval, range);

    const { promise, controllers } = fetchWithProxies(url);
  ...
  // Sequential, not parallel: only the primary proxy fires. The fallback
  // is only attempted if the primary actually fails — never as a
  // simultaneous "just in case" duplicate request. This roughly halves
  // total request volume against the free proxies compared to racing
  // multiple at once, which was contributing to their rate-limiting.
  //
  // Respects SETTINGS.useProxy: when off, this makes exactly one direct
  // request with no proxy involved at all — useful for testing whether a
  // given browser/network actually needs the proxy (e.g. a CORS-unblock
  // extension, or a future environment where Yahoo does send CORS
  // headers). Direct mode will usually fail fast with a CORS network
  // error if the browser blocks it — that's expected, not a bug.
  const FETCH_TIMEOUT_MS = 15000; // was 8000 — too tight for a slow-but-working free proxy

  function fetchWithProxies(targetUrl) {
    if (!SETTINGS.useProxy) {
      const controller = new AbortController();
      return { promise: fetchWithTimeout(targetUrl, FETCH_TIMEOUT_MS, controller), controllers: [controller] };
    }

    const controllers = CORS_PROXIES.map(() => new AbortController());
    let chain = Promise.reject();
    CORS_PROXIES.forEach((buildUrl, i) => {
      chain = chain.catch(() => fetchWithTimeout(buildUrl(targetUrl), FETCH_TIMEOUT_MS, controllers[i]));
    });
    return { promise: chain, controllers };
  }
